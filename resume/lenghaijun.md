...
* 上海幻电信息科技有限公司(上海哔哩哔哩)
* 早起的码农, 不是太喜欢玩游戏但热爱游戏开发中的技术钻研和攻艰
...
* `Unity3D` 编辑器, 独立完成关卡编辑器第一版, 包括基本瓦片地形编辑, 道具布置编辑, 怪物布置编辑 (童话少女)
* `Unity3D` 编辑器插件, 如: 将 `excel` 读写及 `lua/json` 转换, `google-protocol-buf` 编译等工具集成到 `UnityEditor` 中, 方便了大家的工作, 提高了效率 (童话少女)
* `Unity3D` 热更新, `lua` 脚本和资源热更新打包发布及加载系统 (童话少女)
* `Unity3D` 移动SDK接入, 如: `BiliBili SDK iOS` 用户系统接入 (童话少女)
* `Unity3D` 游戏模块，如: 核心战斗中的技能模块 (童话少女)
* `Unreal` 引擎，业余研究过部分源码并自主编译过引擎核心和编辑器，在示例基础上做过探索实验，了解 `UnrealScript` 及蓝图开发模式
