...
|雇主: 上海幻电信息科技有限公司（哔哩哔哩） |

----
## 专业积累 ##

**@编程环境** 能够熟练配置及运用 `UNIX`/`Windows` 环境编程, 研读过 `UNIX 环境高级编程` 等著作, 对操作系统原理有一定深入的理解.

**@引擎框架** 掌握 `cocos2d-x/js/lua` 引擎的使用和扩展及支持工具的使用; 长期关注 `GitHub`/`BitBucket`相关领域开源项目; 熟悉 `Unity3D` 引擎的使用; 了解 `Unreal`, `OGRE` 等引擎, 阅读过其部分源码; 对经典公共类库, 如 `boost`/`websocekt++`/`php`/`MySQL`/`SQLite` 等也有一定深入的探索研究和使用经验, 对于陌生类库能够快速根据文档或示例面向接口编程. 

**@效能工具** 熟练使用 `Xcode`/`Visual Studio`/`Eclipse`/`vim`/`cmake` 等主流编程开发工具, 掌握高级 `shell` 脚本编程, 善于寻找学习运用各种效率工具, 熟练使用 `git`/`hg`/`svn`/`docker` 等版本控制迭代部署工具.

**@编程算法** 对数据结构和算法有一定深入的研究并能应用到项目中, 如树和图的操作, 常用排序算法, A\* 寻路算法, 加密算法等.

**@目标平台** 有多个完整的主流手机平台 `iOS`/`Android`/`WindowsPhone 8` 从生产开发到交付经验, 能熟练提交应用到各个平台应用商店.

**@战力输出** 即能够单打独斗, 也乐意与优秀队友并肩作战, 相互交流分享, 共同成长. 热爱钻研, 以解决难题为乐, 攻艰能力多次得到团队认可.

**@代码风格** 曾尝试过多种代码风格, 最终喜欢 `cocoa` 的代码风格, 如果有必要我也乐意顺应团队的约定风格和规范. 我从事游戏开发的年限不算很长, 但我进入这个世界就无法自拔, 看到一款游戏从自己手中一步步成长诞生, 呈现给玩家, 并得到他们的认可远比在别人的游戏中获得的乐趣更让人开心.

**@自我评价** 生于天府, 成于西域, 物竞天择, 适者生存. 从边疆艰苦环境中出来的我, 在经历的每个环境都能快速适应, 力争上游, 相信这种能力能够帮我快速适应新工作, 充分发挥自己才智.
...
> **工具开发**, 如: Excel 转表工具开发，Unity 集成；

> **自动化**, 配置基于 `Jenkins` 的自动化构建系统, 可同步输出 android 和 iOS 安装包。
...
**项目职责** 
> **重构优化**, 推行 MVC 设计模式, 模块化拆分已便动态加载游戏内容, 将初期 600M 内存占用降至峰值 300M, 并结合预加载和缓存技术平衡卡顿, 拆分 AI 和行为的逻辑与参数；

> **功能开发**, 如:技能符文系统, 任务系统等；

> **引擎维护**, 如:升级引擎, 修复引擎错误, 原生控件和功能封装并绑定导出 Lua 接口等；

> **基础设施**, 设计实现基于 `Jenkins` 的自动化构建系统, 可同步输出 android 和 iOS 安装包, 并可根据版本控制系统一键生成并自动部署热更新补丁到 OSS对象存储服务 或 web 站点；架设基于 `redmine` 的项目管理系统, 推行失败。
...
**项目职责** 
> **系统开发**, 如: 采用标准 `MVC` 模式, `socket` + `ByteBuffer` 通信协议实现了试练之塔, 世界 boss, 十二星宫, 跨服组队战等若干玩家喜爱的系统, 在游戏文本聊天室中无缝嵌入语音聊天室;

> **系统优化**, 如: 运用多线程技术减少游戏启动场景加载时间, 使用本地缓存还原系统再入状态等提升用户体验;

> **引擎维护** `cocos2d-x`, 如: 添加可配置悬浮控件, 支持单击, 双击, 长按, 拖动等操作, 修复重叠控件事件响应问题, 客户端日志保存回传, 运用 `OpenSSH` 的 `RC4` 算法对资源文件进行加解密, 64位 `iOS` 系统兼容等;

> **编辑器扩展维护**, 如: 添加样式模版, 编辑快捷键, 添加 `复制` & `粘贴` 功能;

> **iOS 第三方 SDK 接入**, 如: 快用, PP助手等渠道接入;
...
### 上海灼焰信息科技有限公司/上海幻电信息科技有限公司 | 2015/10 -- 至今
行业类别: 网络游戏 

工作描述: 参与开发 `魔法少女小圆` 手机游戏开发
...
**详细描述** 项目组间经验分享, 在『萌果学院』中交流开发技巧和项目经验. 如不同机型屏幕适配方案, `jsb` 项目开发流程, 新版引擎特性等. 本人在其中分享了 `多线程异步加载和预加载技巧`, 和 `WindowsPhone 8` 移植过程中的一些探索经验.
...
## 其他 ##
**语言能力**
> 英语: CET-4 能够流畅查阅英文手册及博客论坛, 编写简洁文档.
